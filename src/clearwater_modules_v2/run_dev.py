...
#rename columns
air_temperature = air_temperature.rename(columns={"Datetime": "time", "TairC": "air_temperature"})
#convert to datetime
...
cloudiness = pd.DataFrame({"time" : minute_5, "cloudiness" : np.ones(minute_5.size)*0.1})
wind_speed = pd.DataFrame({"time" : minute_5, "wind_speed" : np.ones(minute_5.size)*3})
atmospheric_pressure = pd.DataFrame({"time" : minute_5, "atmospheric_pressure" : np.ones(minute_5.size)*1013.0})
...
model = Model(
    ...
    processes=(riverine_process, Temperature()),
    variables=variables,
