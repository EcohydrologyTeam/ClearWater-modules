...
class BenthicAlgae:
    ...
    def Calculations(self):
        ...
        mub_max_tc = TempCorrection(self.Balgae_constant['mub_max'], 1.047).arrhenius_correction(
            self.global_vars['TwaterC'])  # TODO Need to define

        # Respiration rate (1/day), Range {0.1 - 0.8}
        krb_tc = TempCorrection(self.Balgae_constant['krb'], 1.06).arrhenius_correction(
            self.global_vars['TwaterC'])  # TODO Need to define

        # Mortality rate (1/day), Range {0.0 - 0.8}
        kdb_tc = TempCorrection(self.Balgae_constant['kdb'], 1.047).arrhenius_correction(
            self.global_vars['TwaterC'])  # TODO Need to define

        # Ratio of nitrogen to bottom algal biomass (mg-N/mg-D)
        self.rnb = self.Balgae_constant['BWn']/self.Balgae_constant['BWd']
        # Ratio of phosphorus to bottom algal biomass (mg-P/mg-D)
        self.rpb = self.Balgae_constant['BWp']/self.Balgae_constant['BWd']
        # Ratio of carbon to bottom algal biomass        (mg-C/mg-D)
        self.rcb = self.Balgae_constant['BWc']/self.Balgae_constant['BWd']
        # Ratio of chlorophyll-a to bottom algal biomass (µg-Chl-ab/mg-D)
        self.rab = self.Balgae_constant['BWa']/self.Balgae_constant['BWd']
        ...
        KEXT = math.exp(-self.global_vars['lambda']
        ...
        if self.global_vars['Ab'] <= 0.0 or KEXT <= 0.0 or self.global_vars['PAR'] <= 0.0:
            ...
            FLb = self.global_vars['PAR'] * KEXT / \
                (self.Balgae_constant['KLb'] + self.global_vars['PAR'] * KEXT)
            ...
            FLb = self.global_vars['PAR'] * KEXT / self.Balgae_constant['KLb'] * math.exp(
                1.0 - self.global_vars['PAR'] * KEXT / self.Balgae_constant['KLb'])
        ...
        if self.global_module_choices['use_NH4'] or self.global_module_choices['use_NO3']:
            FNb = (self.global_vars['NH4'] + self.global_vars['NO3']) / (
                self.Balgae_constant['KsNb'] + self.global_vars['NH4'] + self.global_vars['NO3'])
            if math.isnan(FNb):
                FNb = 0.0
            if FNb > 1.0:
                FNb = 1.0
        ...
        if self.global_module_choices['use_TIP']:
            FPb = self.global_vars['fdp'] * self.global_vars['TIP'] / (
                self.Balgae_constant['KsPb'] + self.global_vars['fdp'] * self.global_vars['TIP'])
            if math.isnan(FPb):
                FPb = 0.0
            if FPb > 1.0:
                FPb = 1.0
        ...
        FSb = 1.0 - \
            (self.global_vars['Ab'] /
             (self.global_vars['Ab'] + self.Balgae_constant['Ksb']))
        if math.isnan(FSb):
            FSb = 1.0
        if FSb > 1.0:
            FSb = 1.0
