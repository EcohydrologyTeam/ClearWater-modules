...
class Phosphorus:
    ...
    def Calculation(self):
        ...
        if self.global_module_choices['use_OrgP']:
            kop_tc = TempCorrection(self.P_constants['kop'], 1.047).arrhenius_correction(
                self.global_vars['TwaterC'])

        if self.global_module_choices['use_TIP'] and not self.global_module_choices['use_SedFlux']:
            rpo4_tc = TempCorrection(self.P_constants['rpo4'], 1.074).arrhenius_correction(
                self.global_vars['TwaterC'])
        ...
        if self.global_module_choices['use_OrgP']:
            ...
            OrgP_Settling = (
                self.global_vars['vsop'] / self.global_vars['depth']) * self.global_vars['OrgP']

            if self.global_module_choices['use_Algae']:
                ApDeath_OrgP = self.algae_pathways['rpa'] * \
                    self.algae_pathways['ApDeath']
            else:
                ApDeath_OrgP = 0

            if self.global_module_choices['use_BAlgae']:
                AbDeath_OrgP = (self.Balgae_pathways['rpb'] * self.Balgae_pathways['Fw'] *
                                self.Balgae_pathways['Fb'] * self.Balgae_pathways['AbDeath']) / self.global_vars['depth']
            else:
                AbDeath_OrgP = 0.0
        ...
        if self.global_module_choices['use_TIP']:
            if self.global_module_choices['use_SedFlux']:
                DIPfromBed = self.sedFlux_pathways['JDIP'] / \
                    self.global_vars['depth']
            else:
                DIPfromBed = rpo4_tc / self.global_vars['depth']

            TIP_Settling = self.global_vars['vs'] / self.global_vars['depth'] * (
                1.0 - self.global_vars['fdp']) * self.global_vars['TIP']
            ...
            if self.global_module_choices['use_Algae']:
                DIP_ApRespiration = self.algae_pathways['rpa'] * \
                    self.algae_pathways['ApRespiration']
                DIP_ApGrowth = self.algae_pathways['rpa'] * \
            ...
            if self.global_module_choices['use_BAlgae']:
                DIP_AbRespiration = self.Balgae_pathways['rpb'] * \
                    self.Balgae_pathways['AbRespiration']
                DIP_AbGrowth = self.Balgae_pathways['rpb'] * self.Balgae_pathways['Fb'] * \
                    self.Balgae_pathways['AbGrowth'] / \
                    self.global_vars['depth']
            else:
        ...
        if self.global_module_choices['use_TIP']:
            ...
            # TODO Check formula, check all with the change of d/dt
            DIP = (self.global_vars['TIP']) * self.global_vars['fdp']
