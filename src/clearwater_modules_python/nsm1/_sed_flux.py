...
class SedFlux:
    ...
    def __init__(self, global_module_choices, global_vars, sedFlux_constant_changes, CBOD, ksbod_tc, algae_pathways, Balgae_pathways):
        self.global_module_choices = global_module_choices
        self.global_vars = global_vars
        # TODO ksbod_tc has multiple values based on differernt groups, array not dictionary
        self.ksbod_tc = ksbod_tc
        self.algae_pathways = algae_pathways
        self.Balgae_pathways = Balgae_pathways
        # TODO CBOD has multiple values based on different groups (up to 10) and their differernt decay rates, array not dictionary
        self.CBOD = CBOD
        ...
        self.sedFlux_constants = {
            ...
            'KsSO4': 1.08,
            'Css2': 0.5,
            'vb': 0.0025,  # TODO Make sure not a global parameter
            'h2': 0.1,  # TODO Make sure not a global parameter
  ...
  # local variables
    ...
    """
        ...
        vb = self.sedFlux_constants['vb']
        h2 = self.sedFlux_constants['h2']
        ...
        # TODO check this, why this too? was in the subroutine that called all the subroutines
        DOX = max(DOX, 0.01)
        ...
        for i in (1, self.global_vars['nCBOD']):
            ...
            JPOC[3] = JPOC[3] + self.ksbod_tc[i] * \
                (1.0 - FCBOD1 - FCBOD2) * self.CBOD[i] / roc
    ...
    # POC G1/G2/G3 in the second layer. Able for diagenesis and depends on depositional flux above
        for i in (1, 3):
            ...
            if self.sedFlux_constants['SedFlux_solution_option'] == 1:
                POC2[i] = JPOC[i] / (KPOC_tc[i] * h2 + vb)
                PON2[i] = JPON[i] / (KPON_tc[i] * h2 + vb)
                POP2[i] = JPOP[i] / (KPOP_tc[i] * h2 + vb)
            # unsteady state solution
            elif (self.sedFlux_constants['SedFlux_solution_option'] == 2):
                POC2[i] = (JPOC[i] + POC2[i] * h2 / dt) / \
                    (h2 / dt + KPOC_tc[i] * h2 + vb)
                PON2[i] = (JPON[i] + PON2[i] * h2 / dt) / \
                    (h2 / dt + KPON_tc[i] * h2 + vb)
                POP2[i] = (JPOP[i] + POP2[i] * h2 / dt) / \
                    (h2 / dt + KPOP_tc[i] * h2 + vb)
        ...
        if SedFlux_solution_option == 1:
            # TODO check: Benthic stress does not accumulate when temperature is lower than TempBen=10oC.
            ST = 0.0
        elif SedFlux_solution_option == 2:
        ...
        # Oxygen attenuation factor for sediment nitrification
        ...
        if (Salinity > 0.01):
            ...
            SO4 = (20.0 + 27.0 / 190.0 * 607.445 * Salinity) * roso4
        else:
        ...
        # Sulfate reduction depth method
            ...
            for i in (1, int(maxit)):
                ...
                b1 = KL01 * NH4
                TNH41, TNH42 = MatrixSolution(
                    TNH41, TNH42, a11, a12_TNH4, b1, a21_TNH4, a22_TNH4, b2_TNH4)
                TNH41 = max(TNH41, 0.0)
                TNH42 = max(TNH42, 0.0)
                ...
                # TODO TODO call MatrixSolution(NO31, NO32, a11, a12_NO3, b1, a21_NO3, a22_NO3, b2_NO3)
                NO31 = max(NO31, 0.0)
                NO32 = max(NO32, 0.0)

                JC_dn = rondn * (vno31_tc * vno31_tc /
                                 KL01 * NO31 + vno32_tc * NO32)
                ...
                if POCdiagenesis_part_option == 1:
                    ...
                    if (HSO4 > h2):
                        HSO4 = h2
                    if HSO4 == 0.0:
                        # set a large value (1) for KL12SO4.
                        KL12SO4 = 1.0
                    else:
                    ...
                    # 0 = bx(4) + ad(4,3) * TH2S1 + ad(4,4) * TH2S2 + JCc * SO42 / (SO42 + KsSO4)
                    ...
                    ad[0][1] = KL12SO4
                    ad[0][2] = con_sox / KL01
                    ad[2][2] = - vb - fps1 * w12 - fds1 * \
                        KL01 - fds1 * KL12SO4 - con_sox / KL01
                    ...
                    g[1] = ((bx[0] * ad[2][2] - ad[0][2] * bx[2]) * ad[3][3] -
                            bx[0] * ad[2][3] * ad[3][2]) / (ad[0][2] * ad[2][3]) + bx[3]
                    g[0] = g[1] + bx[1]
                    h[1][0] = ad[0][0] * (ad[2][2] * ad[3][3] -
                                          ad[2][3] * ad[3][2]) / (ad[0][2] * ad[2][3])
                    h[1][1] = ad[0][1] * (ad[2][2] * ad[3][3] -
                                          ad[2][3] * ad[3][2]) / (ad[0][2] * ad[2][3])
                    h[0][0] = h[1][0] + ad[1][0]
                    h[0][1] = h[1][1] + ad[1][1]
                    ...
                    ra1 = h[0][0] * g[1] - h[1][0] * g[0] + \
                        (h[0][0] * h[1][1] - h[0][1] *
                         h[1][0]) * KsSO4 + h[0][0] * JCc
                ...
                # sulfate reduction depth method
                    ...
                    if (SO4 <= 0.1):
                        ...
                        if SedFlux_solution_option == 1:
                            ...
                            ra0 = - 2.0 * Dd_tc * \
                                (KL01 * SO4 + con_sox / KL01 * TH2S1_prev)
                        elif SedFlux_solution_option == 2:
                            ra2 = 2.0 * KL01 * JCc / h2 + \
                                (KL01 * SO4 + con_sox / KL01 * TH2S1_prev) / dt
                            ra1 = 2.0 * Dd_tc * JCc / h2 - 2.0 * KL01 * HSO4_prev * SO42_prev / dt
                            ra0 = - 2.0 * Dd_tc * \
                                (KL01 * SO4 + con_sox / KL01 *
                                 TH2S1_prev + HSO4_prev * SO42_prev / dt)

                        HSO4 = (- ra1 + math.sqrt(ra1 * ra1 -
                                4.0 * ra2 * ra0)) / 2.0 / ra2
                        if (HSO4 > h2):
                            HSO4 = h2
                        ...
                        # TODO, TODO call MatrixSolution(TH2S1, TH2S2, a11, a12_TH2S, b1, a21_TH2S, a22_TH2S, b2)
                ...
                if Methane_solution_option == 1:
                    CSODmax = min(
                        math.sqrt(2.0 * KL12 * CH4sat * JCc_CH4), JCc_CH4)
                    # vch41_tc = Arrhenius_TempCorrection(vch41(r), TsedC)            #TODO figure out the different temperature correctiontemperature correction
                    if vch41_tc / KL01 < 100.0:
                        CSOD_CH4 = CSODmax * \
                            (1.0 - 2.0 / (math.exp(vch41_tc / KL01) +
                             math.exp(-vch41_tc / KL01)))
                    else:
                        CSOD_CH4 = CSODmax
                ...
                # numerical solutions
                    ...
                    # TODO TODO call MatrixSolution(CH41, CH42, a11, a12_CH4, b1, a21_CH4, a22_CH4, b2)
                    if CH42 > CH4sat:
                        CH42 = CH4sat
            ...
            # PNH41_PNH42 = w12 * (fp2 * TNH42 - fp1 * TNH41)
            ...
            # PH2S1_PH2S2 = w12  * (TH2S2 * fps2 - TH2S1 * fps1)
            ...
            if Methane_solution_option == 1:
                ...
                JCH4 = KL01 * (CH41 - CH4)
                if CH42 == CH4sat:
                    JCH4g = JCc_CH4 - JCH4 - CH41_Oxidation - \
                        (CH42 - CH42_prev) / dt * h2
                else:
                    JCH4g = 0.0
            ...
            if SedFlux_solution_option == 1:
                ...
                a22 = KL12 + h2 / dt
                b2 = (JCc_CH4 / 2.0 + JCc_SO4) / roc + \
                    rcdn * NO32_Denit + DIC2 * h2 / dt

            # TODO TODO call MatrixSolution(DIC1, DIC2, a11, a12, b1, a21, a22, b2)
            DIC1 = max(DIC1, 0.0)
            DIC2 = max(DIC2, 0.0)
            ...
            # TODO TODO call MatrixSolution(TIP1, TIP2, a11, a12, b1, a21, a22, b2)
            TIP1 = max(TIP1, 0.0)
            TIP2 = max(TIP2, 0.0)
