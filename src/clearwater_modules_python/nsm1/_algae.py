...
class Algae:
    ...
    def Calculations(self):
        ...
        rna = self.algae_constant['AWn'] / self.algae_constant['AWa']
        # Algal P : Chla ratio [mg-P/ugChla]
        rpa = self.algae_constant['AWp'] / self.algae_constant['AWa']
        # Algal C : Chla ratio [mg-C/ugChla]
        rca = self.algae_constant['AWc'] / self.algae_constant['AWa']
        # Algal D : Chla ratio [mg-D/ugChla]
        rda = self.algae_constant['AWd'] / self.algae_constant['AWa']

        # Parameters related to algae growth and settling
        # Light limiting constant for algal growth [W/m^2]
        KL = self.algae_constant['KL']
        ...
        mu_max_tc = TempCorrection(self.algae_constant['mu_max'], 1.047).arrhenius_correction(
            self.global_vars['TwaterC'])           # Maximum algal growth rate [1/d]
        krp_tc = TempCorrection(self.algae_constant['krp'], 1.047).arrhenius_correction(
            self.global_vars['TwaterC'])                 # algae respiration rate [1/d]
        kdp_tc = TempCorrection(self.algae_constant['kdp'], 1.047).arrhenius_correction(
            self.global_vars['TwaterC'])                 # algae mortality rate [1/d]
        ...
        KEXT = (self.global_vars['lambda'] * self.global_vars['depth'])
        ...
        if self.global_module_choices['use_NH4'] or self.global_module_choices['use_NO3']:
            ...
            # [unitless]
            FN = (NH4 + NO3) / (self.algae_constant['KsN'] + NH4 + NO3)
            if math.isnan(FN):
                FN = 0.0
            if FN > 1.0:
                FN = 1.0
        ...
        if self.global_module_choices['use_TIP']:
            # [unitless]
            FP = fdp * TIP / (self.algae_constant['KsP'] + fdp * TIP)
            if math.isnan(FP):
                FP = 0.0
            if FP > 1.0:
                FP = 1.0
        ...
        # Algal settling
        # [ug-Chla/L/d]
        ApSettling = self.algae_constant['vsap'] / \
            self.global_vars['depth'] * Ap

        # Algal Biomass Concentration
        # dA/dt = A*(AlgalGrowthRate - AlgalRespirationRate - AlgalDeathRate - AlgalSettlingRate)(mg/L/day)
        dApdt = ApGrowth - ApRespiration - \
            ApDeath - ApSettling     # [ug-Chla/L/d]
