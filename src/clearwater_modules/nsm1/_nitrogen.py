...
class Nitrogen:
    ...
    def Calculations(self):
        ...
        if self.global_module_choices['use_NH4']:
            knit_tc = TempCorrection(self.nitrogen_constant['knit'], 1.083).arrhenius_correction(
                self.global_vars['TwaterC'])

        if not self.global_module_choices['use_SedFlux']:
            rnh4_tc = TempCorrection(self.nitrogen_constant['rnh4'], 1.074).arrhenius_correction(
                self.global_vars['TwaterC'])
            vno3_tc = TempCorrection(self.nitrogen_constant['vno3'], 1.08).arrhenius_correction(
                self.global_vars['TwaterC'])

        if self.global_module_choices['use_OrgN']:
            kon_tc = TempCorrection(self.nitrogen_constant['kon'], 1.074).arrhenius_correction(
                self.global_vars['TwaterC'])

        if self.global_module_choices['use_NO3']:
            kdnit_tc = TempCorrection(self.nitrogen_constant['kdnit'], 1.045).arrhenius_correction(
                self.global_vars['TwaterC'])
    ...
    #  Calculating Nitrogen Kinetics
        if self.global_module_choices['use_Algae'] and self.global_module_choices['use_NH4'] and self.global_module_choices['use_NO3']:
            ApUptakeFr_NH4 = self.nitrogen_constant['PN'] * self.global_vars['NH4'] / (self.nitrogen_constant['PN'] * self.global_vars['NH4'] + (
                1.0 - self.nitrogen_constant['PN']) * self.global_vars['NO3'])          # [unitless]
    ...
    # Check for benthic and recompute if necessary
        if self.global_module_choices['use_BAlgae'] and self.global_module_choices['use_NH4'] and self.global_module_choices['use_NO3']:
            AbUptakeFr_NH4 = (self.nitrogen_constant['PNb'] * self.global_vars['NH4']) / (
                self.nitrogen_constant['PNb'] * self.global_vars['NH4'] + (1.0 - self.nitrogen_constant['PNb']) * self.global_vars['NO3'])
    ...
    # Check if NH4 and NO3 are very small.  If so, force uptake_fractions appropriately.
     ...
     dOrgN/dt =   Algae_OrgN              (Floating Algae -> OrgN)
        ...
        if self.global_module_choices['use_OrgN']:
            ...
            OrgN_Settling = self.global_vars['vson'] / \
                self.global_vars['depth'] * self.global_vars['OrgN']

            if self.global_module_choices['use_Algae']:
                ApDeath_OrgN = self.algae_pathways['rna'] * \
                    self.algae_pathways['ApDeath']
            else:
                ApDeath_OrgN = 0.0

            if self.global_module_choices['use_BAlgae']:
                AbDeath_OrgN = self.Balgae_pathways['rnb'] * self.Balgae_pathways['Fw'] * \
                    self.Balgae_pathways['Fb'] * \
                    self.Balgae_pathways['AbDeath'] / self.global_vars['depth']
            else:
                AbDeath_OrgN = 0.0
