...
1. ``test_baseline_parity_bit_identical`` — replays the Phase 0
   ...
   ``baseline_coupled_trajectory_d82a5ed.nc`` bit-identically when no
   ``REGISTRY_DIAGNOSTICS`` names are pre-registered. This is the
   §11.2 contract enforced inside the test suite. (Active baseline at
   ``d82a5ed``, which wires BenthicAlgae's computed light-extinction
   coefficient (utils.light.L) on top of ``e0185de`` (FloatingAlgae
   light-extinction); the benthic light limitation now uses the
   optical-constituent lambda, a broad cascade through algae/DO. Prior
   baselines ``e0185de`` (FloatingAlgae light-extinction), ``d530a3a``
   (Pathogen algae_floating), ``6c10f36`` (NSM1-CA-1/SCI-N1/SCI-A3/SCI-A2),
...
_BASELINE_NETCDF = (
    ...
    / "baseline"
    / "baseline_coupled_trajectory_d82a5ed.nc"
)
