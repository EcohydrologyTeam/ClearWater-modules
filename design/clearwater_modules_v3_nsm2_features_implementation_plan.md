...
**Reference:** CE-QUAL-W2 carbonate routine; Chapra (1997) Ch. 22; Stumm & Morgan. NSM1 v3 already carries `Alk` as a **tracer** with source/sink terms (nitrification consumption, denitrification production, algal growth/respiration coupling — NSM1 design spec line 645). Step 4 **adds the equilibrium solver on top of the existing source/sink terms; it does not replace them** — with one scoped exception: the **algal/benthic-algae alkalinity term is reformulated from its carbon-routed basis to the nitrogen-flux basis (NSM1-SCI-A1)**, see below.

**NSM1-SCI-A1 (gold-standard deferral — carbon-routed → N-flux algal alkalinity).** The NSM1 v3 algal/benthic alkalinity coupling is computed on a *carbon-routed* basis (algal C flux × stoichiometry × NH4/NO3 split). CE-QUAL-W2 (`water-quality.f90`) and Stumm & Morgan compute it from the algal **nitrogen** flux directly; the two agree only at exact Redfield N:C, and NSM1's `AWn/AWc` is ~2% off, so the algal alkalinity term carries a ~2% basis error wherever algal uptake drives alkalinity. NSM1-CA-1 (v3 1.0) fixed the carbon-ratio *magnitude*; SCI-A1 is the residual carbon-vs-nitrogen *basis* error, explicitly deferred here from the v3 1.0 gold-standard gate (documented at `Alkalinity` docstring and `parameter_defaults_corrections.md` §3.11). Reformulating to the N-flux basis is naturally bundled with Phase S4-3 — it touches the same `Alk` source/sink code and the same CE-QUAL-W2 / Stumm & Morgan references the solver validates against. It is a separable sub-deliverable of S4-3 (a term reformulation + non-shared-path regression + W2 cross-check), independent of the additive carbonate solver itself. Landing SCI-A1 perturbs the coupled-demo `alkalinity` trajectory only and triggers a documented `alkalinity`-only re-baseline (§7 Tier-2; §1.1 re-baseline policy) — the NSM1-CA-1 / NSM1-SCI-N1 precedent.
...
4. **CO₂ atmospheric exchange:** `k_CO2 = k_O2 · (Sc_CO2/Sc_O2)^(−n)`, where `n` matches the Schmidt exponent of the selected NSM1 reaeration formula (`n = 1/2` for surface-renewal / rough-surface forms, `2/3` for stagnant-film / smooth forms) and tracks the reaeration-menu selection rather than being fixed; reuses the existing NSM1 v3 reaeration menu (river-appropriate piston velocity); flux `k_CO2·([CO2*]_sat − [CO2*])` updates DIC.
...
6. **CO₂ growth limitation:** `f_CO2 = [CO2*] / (KsCO2 + [CO2*])`, wired into the **Step-1 multi-group growth-limitation** chain (multiplicative/min/harmonic, per `growth_rate_option`); it enters algal growth lagged one sub-step (see §5). This closes Step-1 non-goal #7 ("pH-driven CO₂ limitation … deferred to v3.x when alkalinity/pH lands") — the more defensible bloom-collapse mechanism.
...
- **Si growth limitation:** for groups with Step-1 `si_limitation_option[g] = 2`, `f_Si = DSi/(KsSi[g] + DSi)` enters the multi-group growth-limitation chain alongside N, P, light, and (4.B) CO₂ as the previous sub-step's value (lagged coupling; see §5). Groups with `si_limitation_option=1` are Si-unlimited (current behavior) — backward compatible.
...
Because Algae runs before Silica (4.D) and before the carbonate solver (4.B, last), `f_Si` and `f_CO2` enter the algal growth-limitation chain as the previous sub-step's values. This one-sub-step lag is intentional and consistent with the Forward-Euler operator split (the same lag already applies to other inter-process couplings in NSM1 v3); it is not an ordering defect. The sub-step size keeps the lag immaterial; a within-step iteration or moving Algae after the solver is rejected as unnecessary complexity for the river timescales in scope.
...
#### Phase S4-3 — Alkalinity/pH carbonate solver (3.5–4.5 days, incl. SCI-A1)
Equilibrium solver on top of the existing NSM1 `Alk` source/sink terms; `pH`, `[CO2*]`, speciation, `f_NH3`, CO₂ atmospheric exchange via the reaeration menu; `f_CO2` into the Step-1 growth-limitation chain. Ionic-strength input plumbed (reservoir-ready). **Also (separable sub-deliverable) — NSM1-SCI-A1:** reformulate the algal/benthic-algae alkalinity term from its carbon-routed basis to the **N-flux basis** (as CE-QUAL-W2 `water-quality.f90` / Stumm & Morgan), retiring the ~2% off-Redfield basis error deferred from the v3 1.0 gold-standard gate (§4.B; `parameter_defaults_corrections.md` §3.11). Scoped: a `Alk` source/sink term swap + a non-shared-path regression test + a W2 cross-check (reuses the solver's W2 alignment). ~0.5 day incremental; independent of the additive solver. Landing it perturbs the coupled-demo `alkalinity` trajectory only and triggers a documented `alkalinity`-only re-baseline (§7 Tier-2; §1.1 re-baseline policy). **Deliverable:** carbonate solver validated against analytical equilibria and W2 cross-check; CO₂ limitation toggles cleanly; algal alkalinity on the N-flux basis with the SCI-A1 regression test green.
...
- **Tier 2 — Backward-compat parity:** with `multipool_om=false`, `si_limitation_option=1`, and the carbonate solver off, every state variable is byte/float-equivalent to current NSM1 v3 **except `alkalinity`**. SCI-A1 is *deferred* from the v3 1.0 gate, so v3 1.0 itself has no trajectory change and no re-baseline (`parameter_defaults_corrections.md` §3.11). When SCI-A1 is *implemented here in Phase S4-3*, it reformulates the algal/benthic alkalinity term and perturbs the `alkalinity` trajectory by the ~2% off-Redfield basis (§4.B); that is a deliberate kinetics correction, and landing it triggers a documented coupled-demo re-baseline of the `alkalinity` variable only — the exact precedent of NSM1-CA-1 and NSM1-SCI-N1, each of which changed only `alkalinity`. All other variables remain the strict non-regression contract for existing applications.
